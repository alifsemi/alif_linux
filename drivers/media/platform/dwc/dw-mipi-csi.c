...
static struct R_CSI2 reg = {
	...
	.SCRAMBLING = 0x300,
	.SCRAMBLING_SEED1 = 0x304,
	.SCRAMBLING_SEED2 = 0x308,
};
...
void dw_mipi_csi_set_ipi_color_mode(struct dw_csi *csi_dev)
{
	...
	case MEDIA_BUS_FMT_RGB666_1X18:
		csi_dev->ipi_dt =  CSI_2_RGB666;
		csi_dev->hw.ipi_color_mode = COLOR48;
		break;
	/* RGB 565 */
	case MEDIA_BUS_FMT_RGB565_2X8_BE:
	case MEDIA_BUS_FMT_RGB565_2X8_LE:
			      ...
			      struct v4l2_subdev_format *fmt)
{
	...
	dw_mipi_csi_set_ipi_color_mode(dev);

	/*
	 * Frame-Valid (VSYNC) comes atleast 3 pixel clocks before
	 * Line-Valid (HSYNC/DATA_EN). Hence, setting HSA as 3.
	 */
	dev->hw.hsa = 3;
	dev->hw.hbp = 0;
	 ...
	 */
	dev->hw.hsd = dev->hw.num_lanes * 12 - 10;
	dev->hw.hsd = (dev->hw.hsd * fmt->format.width) / 10 -
		dev->hw.hsa + 1;
...
void dw_mipi_csi_start(struct dw_csi *csi_dev)
{
	...
	if (csi_dev->hw.output == IPI_OUT || csi_dev->hw.output == BOTH_OUT) {
		...
		dev_vdbg(dev, "IPI enable: %s\n",
		 csi_dev->hw.output ? "NO" : "YES");
		dev_vdbg(dev, "video mode transmission type: %s timing\n",
...
int dw_mipi_csi_irq_handler(struct dw_csi *csi_dev)
{
	...
	if (flag_softreset && (csi_dev->hw_version_minor >= 30)) {
		dw_mipi_csi_write(csi_dev, reg.IPI_SOFTRSTN, 0);
		dw_mipi_csi_write(csi_dev, reg.IPI_SOFTRSTN, 1);
	}
...
int dw_mipi_csi_specific_mappings(struct dw_csi *csi_dev)
{
	...
	if (csi_dev->hw_version_major == 1) {
		if (csi_dev->hw_version_minor == 30) {
			...
			reg.DATA_IDS_VC_1 = 0x30;
			reg.VC_EXTENSION = 0xC8;
