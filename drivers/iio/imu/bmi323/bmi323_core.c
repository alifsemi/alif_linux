...
struct bmi323_data {
	...
	struct mutex mutex;
	int watermark;
	__le16 fifo_buff[BMI323_FIFO_FULL_IN_WORDS] __aligned(IIO_ALIGN);
	struct {
		__le16 channels[6];
		s64 ts __aligned(8);
	} buffer;
			       ...
			       enum iio_event_direction dir, int state)
{
	...
	if (state) {
		...
		return bmi323_update_ext_reg(data, BMI323_TAP1_REG,
					     BMI323_TAP1_TIMEOUT_MSK,
					     FIELD_PREP(BMI323_TAP1_TIMEOUT_MSK,
							0));
	}
						 ...
						 char *buf)
{
	...
	raw = FIELD_GET(BMI323_TAP1_TIMEOUT_MSK, reg_value);
						  ...
						  const char *buf, size_t len)
{
	...
	ret = bmi323_update_ext_reg(data, BMI323_TAP1_REG,
				    BMI323_TAP1_TIMEOUT_MSK,
				    FIELD_PREP(BMI323_TAP1_TIMEOUT_MSK, val));
	if (ret)
		return ret;
