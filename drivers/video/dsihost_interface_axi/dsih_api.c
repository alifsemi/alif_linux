...
int mipi_dsih_open(struct mipi_dsi_dev *dev, dsih_color_coding_t color_coding)
{
	...
	else
	{
		...
		case DSI_VERSION_130:
			pr_info("%s:HW Version 1.30a\n", FUNC_NAME);
			break;
		case DSI_VERSION_131:
			pr_info("%s:HW Version 1.31a\n", FUNC_NAME);
			break;
	...
	mipi_dsih_hal_dpi_lp_cmd_en(dev, 0);
	...
	if (!mipi_dsih_dphy_configure(dev, 0, DEFAULT_BYTE_CLOCK))
		return FALSE;
...
int mipi_dsih_dpi_video(struct mipi_dsi_dev *dev)
{
	...
	mipi_dsih_hal_dpi_hline(dev, (uint16_t)video_params->h_total_pixels);
	...
	if (video_params->video_mode == VIDEO_BURST_WITH_SYNC_PULSES)
	{
		...
		pr_debug("INFO: h line time -> %d\n",
				 (uint16_t)video_params->h_total_pixels);
		pr_debug("INFO: video_size -> %d\n", video_size);
	}
	else
	{
		/* Non burst transmission */
		pr_info("DSI INFO: non burst video\n");
		null_packet_size = 0;
		...
		pr_debug("DSI INFO: video_params->h_front_porch -> %d", h_front_porch);
		...
		pr_debug("DSI INFO: bytes_per_pixel_x100 -> %d",
				 bytes_per_pixel_x100);
		pr_debug("DSI INFO: video_params->h_active_pixels -> %d",
				 video_params->h_active_pixels);
		pr_debug("DSI INFO: VIDEO_PACKET_OVERHEAD -> %d",
				 VIDEO_PACKET_OVERHEAD);
		pr_debug("DSI INFO: ratio_clock_xPF -> %d",
				 (uint32_t)ratio_clock_xPF);
		pr_debug("DSI INFO: video_params->no_of_lanes -> %d",
				 video_params->no_of_lanes);
		pr_debug("DSI INFO: video_params->h_total_pixels -> %d",
				 video_params->h_total_pixels);
		pr_debug("DSI INFO: video_params->h_back_porch_pixels -> %d",
				 video_params->h_back_porch_pixels);
		pr_debug("DSI INFO: video_params->h_sync_pixels -> %d",
				 video_params->h_sync_pixels);
		pr_debug("DSI INFO: PRECISION_FACTOR -> %d", PRECISION_FACTOR);
		pr_debug("DSI INFO: total bytes -> %d", total_bytes);
		pr_debug("DSI INFO: bytes_per_chunk -> %d", bytes_per_chunk);

		if (total_bytes >= bytes_per_chunk)
		{
			chunk_overhead = total_bytes - bytes_per_chunk - VIDEO_PACKET_OVERHEAD - NULL_PACKET_OVERHEAD;
			pr_debug("DSI INFO: overhead %d -> enable multi packets", chunk_overhead);

			if (!(chunk_overhead > 1))
			{
				/* MULTI packets */
				pr_debug("DSI INFO: multi packets");
				pr_debug("DSI INFO: video_size -> %d", video_size);
				pr_debug("DSI INFO: video_size_step -> %d", video_size_step);

				for (video_size = video_size_step; video_size < video_params->h_active_pixels; video_size += video_size_step)
				{
					pr_debug("DSI INFO: determine no of chunks");
					pr_debug("DSI INFO: video_size -> %d", video_size);

					remain =
						(((video_params->h_active_pixels * PRECISION_FACTOR) / video_size) % PRECISION_FACTOR);
					pr_debug("DSI INFO: remain -> %d", remain);
					if (remain == 0)
					{
						no_of_chunks = video_params->h_active_pixels / video_size;
						pr_debug("DSI INFO: no_of_chunks -> %d", no_of_chunks);
						...
						if (total_bytes >= (bytes_per_chunk * no_of_chunks))
						{
							...
							pr_debug("DSI INFO: bytes_left -> %d", bytes_left);
							break;
						}
					}
				}
			...
			else
			{
				...
				pr_info("DSI INFO: no multi packets\n");
				pr_debug("DSI INFO: horizontal line time -> %d",
						 (uint16_t)((video_params->h_total_pixels * ratio_clock_xPF) / PRECISION_FACTOR));
				pr_debug("DSI INFO: video size -> %d", video_size);
...
int mipi_dsih_dcs_wr_cmd(struct mipi_dsi_dev *dev, uint8_t vc, uint8_t *params, uint16_t param_length)
{
	...
	case 0x00:
		...
		dev_info(dev->parent_dev, "DCS command byte: 0x%X", params[0]);
		break;
	case 0x35:
		video_params->te = 1;
	case 0x3A:
	...
	case 0x2A:
		...
		dev_info(dev->parent_dev, "DCS command byte: 0x%X", params[2]);
		break;
	default:
		dev_err(dev->parent_dev,
...
int mipi_dsih_gen_wr_packet(struct mipi_dsi_dev *dev, uint8_t vc, uint8_t data_type, uint8_t *params, uint16_t param_length)
{
	...
	dev_info(dev->parent_dev, "param_length: %d", param_length);
	...
	if (param_length > 2)
	{
		...
		dev_info(dev->parent_dev, "word count: 0x%X", word_count);
...
uint16_t mipi_dsih_dcs_rd_cmd(struct mipi_dsi_dev *dev, uint8_t vc, uint8_t command, uint8_t bytes_to_read, uint8_t *read_buffer)
{
	...
	default:
		dev_err(dev->parent_dev, "invalid DCS command 0x%X", command);
		return TRUE;
	}
	return TRUE;
}
								 ...
								 uint8_t *read_buffer)
{
	...
	pr_info("data_type: 0x%X", data_type);
	pr_info("msb_byte: 0x%X", msb_byte);
	pr_info("lsb_byte: 0x%X", lsb_byte);
	pr_info("bytes_to_read: %d", bytes_to_read);
	...
	if (!(timeout < DSIH_FIFO_ACTIVE_WAIT))
	{
		...
		dev_err(dev->parent_dev, "TX READ command timed out");
		return FALSE;
	}
											   ...
											   register_config_t *config, uint16_t config_length)
{
	...
	if (all)
	{
		...
		for (current = R_DSI_HOST_VERSION;
			 current <= R_DSI_HOST_PHY_TMR_LPCLK_CFG;
			 count++, current += (R_DSI_HOST_PWR_UP - R_DSI_HOST_VERSION))
		{
			...
			if ((config_length == 0) || (config == 0) || count >= config_length)
			{
				...
				pr_info("DSI 0x%X:0x%X", current, mipi_dsih_read_word(dev, current));
			}
			else
			{
...
int mipi_dsih_edpi_video(struct mipi_dsi_dev *dev)
{
	...
	if (video_params->lp)
		pr_debug("INFO: all DCS commands are configured to Low-power");
	else
		pr_debug("INFO: all DCS commands are configured to High-speed");
	...
	if (video_params->te)
	{
		pr_debug("INFO: tear effect enabled");
